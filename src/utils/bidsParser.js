...
const ENTITY_REGEX = new RegExp(`(?:^|_)(${ENTITY_KEYS.join('|')})-([a-zA-Z0-9]+)`, 'g');
...
export function validateBIDSStructure(structure, hasDatasetDescription) {
    ...
    for (const [dt, count] of datatypeCounts) {
        if (count < subjectIds.length) {
            warnings.push(`${dt} data found in only ${count} of ${subjectIds.length} subjects.`);
        }
    }
...
export async function parseBIDSDirectory(fileList) {
    ...
    for (const nf of niftiFiles) {
        ...
        if (nf.datatype === 'func' && parsed.suffix === 'bold') {
            const eventsRelative = nf.relativePath.replace(/_bold\.(nii\.gz|nii)$/, '_events.tsv');
    ...
    const { errors, warnings, info } = validateBIDSStructure(bidsStructure, hasDatasetDescription);
