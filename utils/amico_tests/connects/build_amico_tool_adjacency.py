#!/usr/bin/env python3
"""Build a directed tool-to-tool adjacency matrix for AMICO tools.
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Build directed AMICO tool adjacency matrix from subsection graph and tool mapping JSON."
        )
    )
    parser.add_argument(
        "--graph",
        type=Path,
        default=script_dir / "amico_subsection_graph.mmd",
        help="Path to Mermaid subsection graph (.mmd).",
    )
    parser.add_argument(
        "--mapping",
        type=Path,
        default=script_dir / "amico_tool_to_subsection_map.json",
        help="Path to tool mapping JSON (must include byTool).",
    )
    parser.add_argument(
        "--out-json",
        type=Path,
        default=script_dir / "amico_tool_adjacency_matrix.json",
        help="Output path for JSON matrix artifact.",
    )
    parser.add_argument(
        "--out-csv",
        type=Path,
        default=script_dir / "amico_tool_adjacency_matrix.csv",
