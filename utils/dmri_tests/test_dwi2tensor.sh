...
# Test: MRtrix3 dwi2tensor - Diffusion tensor estimation
# CWL: public/cwl/mrtrix3/dwi2tensor.cwl
...
TOOL_NAME="dwi2tensor"
CWL_FILE="$CWL_DIR/mrtrix3/dwi2tensor.cwl"
...
check_file_exists "$OUTPUT_DIR/tensor.mif" "tensor" "$RESULTS_FILE" || PASS=false
check_file_nonempty "$OUTPUT_DIR/tensor.mif" "tensor" "$RESULTS_FILE" || PASS=false

# Step 6: Header checks
echo "--- Header checks ---" | tee -a "$RESULTS_FILE"
if [[ -f "$OUTPUT_DIR/tensor.mif" ]]; then
    check_mif_header "$OUTPUT_DIR/tensor.mif" "tensor" "$RESULTS_FILE" || PASS=false
fi

# Save intermediate for tensor2metric
ensure_intermediate
if [[ -f "$OUTPUT_DIR/tensor.mif" ]]; then
