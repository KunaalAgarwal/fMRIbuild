...
# Test: MRtrix3 mrdegibbs - Gibbs ringing removal
# CWL: public/cwl/mrtrix3/mrdegibbs.cwl
...
TOOL_NAME="mrdegibbs"
CWL_FILE="$CWL_DIR/mrtrix3/mrdegibbs.cwl"
...
check_file_exists "$OUTPUT_DIR/degibbs.nii.gz" "degibbs" "$RESULTS_FILE" || PASS=false
check_file_nonempty "$OUTPUT_DIR/degibbs.nii.gz" "degibbs" "$RESULTS_FILE" || PASS=false

# Step 6: Header checks
echo "--- Header checks ---" | tee -a "$RESULTS_FILE"
if [[ -f "$OUTPUT_DIR/degibbs.nii.gz" ]]; then
